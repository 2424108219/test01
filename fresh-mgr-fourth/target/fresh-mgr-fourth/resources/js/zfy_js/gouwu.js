...
window.onload = function demo() {
	...
	function check() {
		...
		if(oCheck[index].checked) {
			...
			totalMoney += parseInt(oInputCount[index].value) * parseInt(otr[index + 1].cells[2].children[0].innerText);
			flag+= 1;
		} else {
			totalCount -= parseInt(oInputCount[index].value);
			totalMoney -= parseInt(oInputCount[index].value) * parseInt(otr[index + 1].cells[2].children[0].innerText);
	...
	function shanChu() {
		...
		demo();
		otr[index + 1].remove();
		totalamount=oCheck.length;
		document.getElementById("sb").innerHTML=parseInt(totalamount);
		shangpingshu();
	}
	//减按钮功能函数
	function jian() {
		...
		if(oInputCount[index].value != 0) {
			...
			otr[index+1].cells[4].innerText = parseInt(oInputCount[index].value) * parseInt(otr[index+1].cells[2].children[0].innerText);
			if(oCheck[index].checked && oInputCount[index].value != 0) {
				totalCount -= 1;
				totalMoney -= parseInt(otr[index+1].cells[2].children[0].innerHTML);
				Spantotal();
			}
		}
	}
	...
	function jia() {
		...
		if(oCheck[index].checked) {
			totalCount += 1;
			totalMoney += parseInt(otr[index+1].cells[2].children[0].innerText);
			Spantotal();
		}
	}
	function jiesuan(){
